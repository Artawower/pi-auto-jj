...
describe("guard lifecycle", async () => {
	...
	it("re-injects skill after session_compact", async () => {
		const pi = await boot("", false);
		const secondTurn = await pi.fire(
			"before_agent_start",
			{ prompt: "turn 2", systemPrompt: "base" },
			makeCtx(),
		);
		expect(secondTurn).toBeUndefined(); // skill already injected on turn 1

		await pi.fire("session_compact", {}, makeCtx());

		const afterCompact = await pi.fire(
			"before_agent_start",
			{ prompt: "turn 3", systemPrompt: "base" },
			makeCtx(),
		);
		// session_compact resets skillInjected → skill re-appears in system prompt
		expect(afterCompact?.systemPrompt).toContain("pi-jj-auto");
	});
	...
	it("does not classify fd-redirect (2>&1) as mutating", async () => {
		...
		const result = await pi.fire(
			"tool_call",
			{ toolName: "bash", input: { command: "cmd 2>&1 | grep error" } },
			makeCtx(),
		);
	...
	it("does not classify 'node --version' as mutating", async () => {
		...
		expect(result).toBeUndefined(); // safe command, no guard triggered
	});
